...
def Get_mode(mode=ClientConfig.HotWordsReadMode):
    if mode == "file":
        return "file"
    elif mode == "keywords":
        return "keywords"
    else:
        logger.warning("模式不是可选项之内,默认为file模式!")
        return "file"
...
def update_hot_zh():
    ...
    if mode == "file":
        ...
        print(f'已载入 {num_hot_zh:5}[/] 条中文热词')
    elif mode == "keywords":
        num_hot_zh = hot_sub_zh.更新热词词典(ClientConfig.ZH)
        print(f'已载入 {num_hot_zh:5}[/] 条中文热词')
...
def update_hot_en():
    ...
    if mode == "file":
        ...
        print(f'已载入 {num_hot_en:5}[/] 条英文热词')
    elif mode == "keywords":
        num_hot_en = hot_sub_en.更新热词词典(ClientConfig.EN)
        print(f'已载入 {num_hot_en:5}[/] 条英文热词')
...
def update_hot_rule():
    ...
    ''')
        ...
        print(f'已载入 {num_hot_rule:5}[/] 条自定义替换规则')
    elif mode == "keywords":
        num_hot_rule = hot_sub_rule.更新热词词典(ClientConfig.RULE)
        print(f'已载入 {num_hot_rule:5}[/] 条自定义替换规则')
...
def update_hot_kwds():
    ...
    if mode == "file":
        ...
        print(f'已载入 {num_kwd:5}[/] 条日记关键词')
    elif mode == "keywords":
        num_kwd = hot_kwds.do_updata_kwd(ClientConfig.KEYWORD)
        print(f'已载入 {num_kwd:5}[/] 条日记关键词')
...
class HotHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        try:
            ...
            print(f'更新热词失败：{e}', style='bright_red')
