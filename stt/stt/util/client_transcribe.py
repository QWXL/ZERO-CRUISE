...
async def transcribe_send(file: Path):
    ...
    print(f'    正在提取音频', end='\r')
    data = process.stdout.read()
    audio_duration = len(data) / 4 / 16000
    print(f'    音频长度：{audio_duration:.2f}s')
    ...
    while True:
        ...
        print(f'    发送进度：{progress:.2f}s', end='\r')
        if is_final:
            break
...
async def transcribe_recv(file: Path):
    ...
    async for message in websocket:
        ...
        print(f'    转录进度: {message["duration"]:.2f}s', end='\r')
        if message['is_final']:
            break
    ...
    print(f'\033[K    处理耗时：{process_duration:.2f}s')
    print(f'    识别结果：\n[green]{message["text"]}')
